                		...
                		if(confirm("确认添加!")) {
	            			$httpService.post(config.addURL, $scope.form).success(function(data) {
		            			if(data.code=="0000"){
			                		...
			                		eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"添加成功"});	//弹出提示框
		            			}else{
		            				eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"添加失败"});
		            			}
		    	            }).error(function(data) {
		    	            	eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"添加出错"});	//弹出提示框
		                    });
                		}
                	}
	            });
