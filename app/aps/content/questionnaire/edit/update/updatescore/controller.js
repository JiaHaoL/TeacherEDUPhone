          ...
          eventBusService.subscribe(controllerName, controllerName+'.save', function(event, btn) { 
              ...
              for (var i = 0; i < $scope.Ucontentlist.length; i++) {      	 
            	  var contentVo={};
            	      ...
            	      contentVo.contentDef=$scope.Ucontentlist[i].IS_CHECKED;//获取是否选中
            	      contentVo.contentvalue=$scope.Ucontentlist[i].SURVEY_CONTENT_VALUE;//获取分数
            	 ...
            	 $httpService.post(config.updatetitleURL, {data:JSON.stringify(questVo)}).success(function(data) {
            			if(data.code=="0000"){
	                		...
	                		eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"评分题修改成功!"});	                		
            			}else{
            				eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"评分题修改失败!"});
            			}
        		 }).error(function(data) {
	            	eventBusService.publish(controllerName,'appPart.load.modal.alert', {"title":"操作提示","content":"评分题修改出错!"});
                 });   		
	       });
