        ...
        return [
            ...
            function($scope,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
            	...
            	var findCommonQuestionnaire = function() {
            		...
            		$httpService.post(config.findQnListByUserPkURL, $scope.form).success(function(data) {
            			if(data.code == '0000') {
            	...
            	var findEvaluateQuestionnaire = function() {
            		$scope.form.page = JSON.stringify($scope.page);
            		$httpService.post(config.findQnListByClassPkURL, $scope.form).success(function(data) {
            			if(data.code == '0000') {
            				$scope.dataList = data.data;
            				PAGE.buildPage($scope,data);	//处理分页
            				$scope.$apply();
		        ...
		        $scope.write = function(obj) {
		        	if(targetTab == "common") {
		        		if(obj.ANSWER_STATUS == '0') {
	    		        	var m2 = {
    			        		url : "aps/content/questionnaire/target/student/collection/write/config.json?pk="+obj.SURVEY_QUEST_ID+"&userPk="+params.userPk,
    			        		contentName : "content",
    			        		text : "填写问卷",
    			        		icon : "plus"
	        			    }
		        	...
		        	if(targetTab == "evaluate") {
		        		if(obj.ANSWER_STATUS == '0') {
	    		        	var m2 = {
    			        		url : "aps/content/questionnaire/target/student/investigate/write/config.json?pk="+obj.SURVEY_QUEST_ID+
		        ...
		        $scope.inspect = function(obj) {
		        	if(targetTab == "common") {
			        	if(obj.ANSWER_STATUS > 0) {
			        		var m2 = {
				        		url : "aps/content/questionnaire/target/student/collection/read/config.json?pk="+obj.SURVEY_QUEST_ID+"&userPk="+params.userPk,
				        		contentName : "content",
				        		text : "查看问卷",
				        		icon : "file"
			    			}
		        	...
		        	if(targetTab == "evaluate") {
			        	if(obj.ANSWER_STATUS == '1') {
			        		var m2 = {
				        		url : "aps/content/questionnaire/target/student/investigate/read/config.json?pk="+obj.SURVEY_QUEST_ID+"&uuid="+obj.SURVEY_SUBJECTTEACHER_ID+"&userPk="+params.userPk,
				        		contentName : "content",
				        		text : "查看问卷",
				        		icon : "file"
			    			}
