    ...
    define([], function() {
        return [
            ...
            function($scope,$location,$httpService,config,params,$routeParams,eventBusService,controllerName,loggingService) {
            	...
            	var menuHistList= [{"url":config.defaultAppPartSrc}];
	            var histNum = 1 ;
	            
	          //接收模块加载事件
            	eventBusService.subscribe(controllerName, 'appPart.load.content.back', function(event, m2) {
              	  if (m2.contentName === config.contentName) {
	              		var m2 = menuHistList[histNum-2];
	              		console.log(menuHistList);
		            	histNum--;
	            ...
	            $("#"+controllerName+" .zyhd").on("swipeleft",function(){
            		console.log("swipeleft");
            		
            	});
            	
            	$("#"+controllerName+" .zyhd").on("swiperight",function(){
